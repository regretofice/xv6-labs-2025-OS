...
//   freerange(end, (void*)SUPERBASE);  // 修改初始化范围
//   ksuperinit();
// }
...
void ksuperinit() {
  ...
  for (; p + SUPERPGSIZE <= (char*)PHYSTOP; p += SUPERPGSIZE) ksuperfree(p);
}

void ksuperfree(void* pa) {
  struct run* r;

  if (((uint64)pa % SUPERPGSIZE) != 0 || (char*)pa < (char*)SUPERBASE ||
      (uint64)pa >= PHYSTOP)
    panic("ksuperfree");
...
void* ksuperalloc(void) {
  ...
  if (r) ksupermem.freelist = r->next;
  release(&ksupermem.lock);

  if (r) memset((char*)r, 5, SUPERPGSIZE);  // fill with junk
  return (void*)r;
}
